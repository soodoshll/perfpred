...
# batch_size_range = range(16, 129, 16)
# image_size_range = [32, 64, 128, 224]

batch_size_range = range(16, 129)
image_size_range = range(32, 257, 32) 

# model_range = range(1)
# batch_size_range = range(4, 5)
# image_size_range = range(16, 17)

for i in tqdm.trange(10000):
    model = random.choice(model_range)
    batch_size = random.choice(batch_size_range)
    image_size = random.choice(image_size_range)
    ret = subprocess.run(args=f'python nas_data.py {model} {batch_size} {image_size}', shell=True, capture_output=True)
    if ret.returncode == 0:
